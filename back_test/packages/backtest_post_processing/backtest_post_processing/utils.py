...
def get_symbol_from_db(symbol='SPX INDEX', start = None, end = None): # CURRENTLY HARDCODED TO MARKET SERIES
    ...
    query = "select HD_PK from PASS_SYS.V_SERIE where CD_SERIETYPE = 'MKT_DATA' AND ST_SOURCE = '" + source + "' AND ST_SECURITY_CODE='" + symbol + "'"
    cursor.execute(query)
    data = cursor.fetchall()
    # put the source dynamic later
    
#     aux = None
#     for temp_data in data:
#         print temp_data, temp_data[1].split(' / ')
#         if temp_data[1].split(' / ')[1] == 'MARKET DATA' and temp_data[1].split(' / ')[0] == source:
#             aux = temp_data[0]
#     
#     if symbol == 'SPX INDEX':
...
def extract_data_from_zipline(backtest, same_dataframe = True):
    """Extract stuff
    """
    
    list_of_stocks, alloc = whgts.allocation_to_df(backtest.allocation)
    ...
    try:
        ...
        transactions = txn.make_transaction_frame(backtest.transactions, list_of_stocks)
        pos_columns = [list_of_stocks[int(item)] for item in positions.columns[:-1]]
        positions.columns = ['position_' + tmp for tmp in pos_columns] + ['position_cash']
        transactions.index.tz = None
        if same_dataframe:
            return pd.DataFrame(returns).join(alloc).join(positions).join(transactions).join(gross_lev)
        else:
            return returns, positions, transactions, gross_lev, alloc
    except:
        if same_dataframe:
            return pd.DataFrame(returns) #.join(alloc)
        else:
            return returns, positions, transactions, gross_lev, alloc
