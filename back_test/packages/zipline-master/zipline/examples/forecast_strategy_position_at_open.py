...
input_data = database_wrapper_zipline.load_data_from_passdb(fields_tickers, source = 'BLP', timezone_indication = 'UTC', start = start_date, end = None, instruments = params.instruments, set_price = params.set_price, align_dates = True, set_volume_val = 1e6)
...
def initialize(context):
    ...
    context.counter_expiration = 0
    context.counter_trades = 0
    context.counter_success_trades  =0
...
def handle_data(context, data):
    ...
    if params.verbose:
        ...
        print 'Number of trades: ', context.counter_trades
        print 'Success trades: ', context.counter_success_trades
    ...
    for i in xrange(len(fields_tickers)):
        ...
        if not context.is_invested[i]:
            ...
            if context.expected_returns[i] > 0:
                if data[symbol(fields_tickers[i])].close > context.expected_prices[i]:
                    order(symbol(fields_tickers[i]), -params.trade_quantity)
                    context.longed[i] = False  
                    context.is_invested[i] = False   
                    context.counter_trades += 1
                    context.counter_success_trades += 1                  
            else:
                if data[symbol(fields_tickers[i])].close < context.expected_prices[i]:
                    order(symbol(fields_tickers[i]), params.trade_quantity)
