...
def nearcorr(A, tol=[], flag=0, max_iterations=1000, n_pos_eig=0,
             weights=None, verbose=False,
             except_on_too_many_iterations=True, delta = 1e-7, eps = np.spacing(1)):
    """
...
def proj_spd(A, delta):
    ...
    # delta = 10**(np.floor(np.log10(np.min(d[np.where(d>0)[0]]))) +1) 

    A = (v * np.maximum(d, delta)).dot(v.T)
    A = (A + A.T) / 2
    return(A)